...
def get_coverage(sky_image_path, is_daytime):
    ...
    if is_daytime:
        ...
        q = (sky_image.convert("RGB")).quantize(colors=15, method=1, kmeans=0, palette=None)
        q = q.convert("HSV")
...
def get_all(sky_image_path, is_daytime, recipients, image_save_to_dir, frompassword):
    """Returns image with overlaid clouds detected and the cloud coverage percentage
    ...
    frompassword        - password for oswegocloud@gmail.com
    """
    ...
    if spouts_detected:
        ...
        waterspoutalert.send_message(image_save_to_dir + "temp_waterspout.jpg", recipients, frompassword)
